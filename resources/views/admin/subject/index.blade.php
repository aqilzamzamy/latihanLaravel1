        ...
        <div class="mb-4 flex justify-between items-center">
            <form action="{{ route('admin.subjects.index') }}" method="GET" class="flex gap-2">
                <input
                    type="text"
                    name="search"
                    placeholder="Cari mata pelajaran (nama)..."
            ...
            <table class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                <thead class="text-xs uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-300">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center">Nama Mapel</th>
                ...
                <tbody>
                    @forelse ($subjects as $mapel)
                        <tr class="border-t hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            ...
                            <td class="px-6 py-4 text-center">{{ $mapel->name }}</td>
                            <td class="px-6 py-4 text-center">{{ $mapel->description }}</td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.subjects.destroy', $mapel->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-medium shadow-sm transition">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
